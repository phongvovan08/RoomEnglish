<template>
  <div class="example-upload-modal">
    <div class="modal-header">
      <h2>
        <Icon icon="mdi:upload" class="w-6 h-6 mr-3" />
        Upload Ví dụ từ Excel
      </h2>
      ...
      <div class="format-instructions">
        ...
        <div class="columns-info">
          <div class="column-item required">
            <span class="column-name">Sentence</span>
            <span class="column-desc">Câu ví dụ tiếng Anh (bắt buộc)</span>
          </div>
          <div class="column-item required">
            <span class="column-name">Translation</span>
            <span class="column-desc">Bản dịch tiếng Việt (bắt buộc)</span>
          </div>
          <div class="column-item optional">
            <span class="column-name">Grammar</span>
            <span class="column-desc">Giải thích ngữ pháp (tùy chọn)</span>
          </div>
        </div>
      </div>
      ...
      <div v-if="uploadResults" class="upload-results">
        ...
        <div v-if="uploadResults.success" class="success-stats">
          <div class="stat">
            ...
            <span class="stat-value">{{ uploadResults.addedCount }} ví dụ</span>
          </div>
          <div class="stat">
            <span class="stat-label">Đã cập nhật:</span>
            <span class="stat-value">{{ uploadResults.updatedCount }} ví dụ</span>
          </div>
          <div class="stat">
            <span class="stat-label">Lỗi:</span>
            <span class="stat-value">{{ uploadResults.errors?.length || 0 }} ví dụ</span>
          </div>
        </div>
...
const uploadFile = async () => {
  ...
  try {
    ...
    formData.append('vocabularyId', props.vocabularyId.toString())
    
    const response = await fetch('/api/vocabulary-examples/upload-excel', {
      method: 'POST',
...
const downloadTemplate = async () => {
  try {
    const response = await fetch('/api/vocabulary-examples/template.xlsx', {
      headers: createFileUploadHeaders()
    })
    ...
    if (response.ok) {
      ...
      link.href = url
      link.download = 'vocabulary-examples-template.xlsx'
